...
import { getEntities } from './sheikh.reducer';
...
export const Sheikh = () => {
  ...
  const sheikhList = useAppSelector(state => state.sheikh.entities);
  const loading = useAppSelector(state => state.sheikh.loading);
  const totalItems = useAppSelector(state => state.sheikh.totalItems);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        {sheikhList && sheikhList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {sheikhList.map((sheikh, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/sheikh/${sheikh.id}`} color="link" size="sm">
                      {sheikh.id}
                    </Button>
                  </td>
                  <td>{sheikh.firstName}</td>
                  <td>{sheikh.lastName}</td>
                  <td>{sheikh.birthDate ? <TextFormat type="date" value={sheikh.birthDate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/sheikh/${sheikh.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/sheikh/${sheikh.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={sheikhList && sheikhList.length > 0 ? '' : 'd-none'}>
