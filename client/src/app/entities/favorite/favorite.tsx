    ...
    <div>
      ...
      <div className="table-responsive">
        {favoriteList && favoriteList.length > 0 ? (
            ...
            <tbody>
              {favoriteList.map((favorite, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{favorite.learner ? <Link to={`/learner/${favorite.learner.id}`}>{favorite.learner.firstName}</Link> : ''}</td>
                  <td>{favorite.book ? <Link to={`/book/${favorite.book.id}`}>{favorite.book.title}</Link> : ''}</td>
                  <td>{favorite.course ? <Link to={`/course/${favorite.course.id}`}>{favorite.course.title}</Link> : ''}</td>
          ...
          !loading && <div className="alert alert-warning">No Favorites found</div>
        )}
      </div>
      {totalItems ? (
