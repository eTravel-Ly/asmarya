    ...
    <div>
      ...
      <div className="table-responsive">
        {cartItemList && cartItemList.length > 0 ? (
            ...
            <tbody>
              {cartItemList.map((cartItem, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{cartItem.learner ? <Link to={`/learner/${cartItem.learner.id}`}>{cartItem.learner.firstName}</Link> : ''}</td>
                  <td>{cartItem.book ? <Link to={`/book/${cartItem.book.id}`}>{cartItem.book.title}</Link> : ''}</td>
                  <td>{cartItem.course ? <Link to={`/course/${cartItem.course.id}`}>{cartItem.course.title}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/cart-item/${cartItem.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/cart-item/${cartItem.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Cart Items found</div>
        )}
      </div>
      {totalItems ? (
