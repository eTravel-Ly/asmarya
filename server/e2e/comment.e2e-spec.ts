...
describe('Comment Controller', () => {
  ...
  it('/POST create comments', async () => {
    const createdEntity: CommentDTO = (await request(app.getHttpServer()).post('/api/comments').send(entityMock).expect(201)).body;

    expect(createdEntity).toEqual(entityMock);
  });

  it('/PUT update comments', async () => {
    const updatedEntity: CommentDTO = (await request(app.getHttpServer()).put('/api/comments').send(entityMock).expect(201)).body;
  ...
  it('/DELETE comments', async () => {
    const deletedEntity: CommentDTO = (
      await request(app.getHttpServer())
