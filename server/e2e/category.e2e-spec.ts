...
describe('Category Controller', () => {
  ...
  it('/POST create categories', async () => {
    const createdEntity: CategoryDTO = (await request(app.getHttpServer()).post('/api/categories').send(entityMock).expect(201)).body;

    expect(createdEntity).toEqual(entityMock);
  });

  it('/PUT update categories', async () => {
    const updatedEntity: CategoryDTO = (await request(app.getHttpServer()).put('/api/categories').send(entityMock).expect(201)).body;

    expect(updatedEntity).toEqual(entityMock);
  });

  it('/PUT update categories from id', async () => {
    const updatedEntity: CategoryDTO = (
      await request(app.getHttpServer())
        .put('/api/categories/' + entityMock.id)
        .send(entityMock)
        .expect(201)
    ).body;
